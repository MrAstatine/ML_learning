...
parser.add_argument(
    "--print-tree", action="store_true", help="Print the decision tree structure"
)
parser.add_argument(
    "--print-construction",
    action="store_true",
    help="Print detailed tree construction process",
)
...
try:
    ...
    print(
        "Rename your written program as CAMPUS_SECTION_SRN_Lab3.py and run python test.py --ID CAMPUS_SECTION_SRN_Lab3 --data mushroom_data.csv --framework [pytorch|sklearn] [--print-tree] [--print-construction]"
    )
    sys.exit()
...
def evaluate_decision_tree(tree, X_test, y_test, cols, class_names=None):
    ...
    precision_macro, recall_macro, f1_macro = calculate_precision_recall_f1(
    ...
    prediction_rate = (
        valid_predictions / total_predictions if total_predictions > 0 else 0
    )
...
def construct_tree(
    ...
    if level >= max_depth:
        ...
        if target_framework == "pytorch":
            ...
            majority_class = int(
                get_item_value(unique_vals[majority_idx], target_framework)
            )
        else:
        ...
        if print_construction:
            print_node_info(
                f"Hypothesis: Class {majority_class} (max depth reached)", level
            )
        return majority_class
    ...
    if len(used_attributes) >= num_features:
        ...
        if print_construction:
            print_node_info(
                f"Hypothesis: Class {majority_class} (no more attributes)", level
            )
        return majority_class
    ...
    try:
        ...
        available_gains = {
            attr: gain
            for attr, gain in gain_dict.items()
        ...
        if print_construction:
            print_node_info(
                f"Selected Attribute: {cols[selected_attribute]} (gain: {available_gains[selected_attribute]:.4f})",
                level,
            )
        ...
        for value in unique_values:
            ...
            if len(subset_data) == 0:
                ...
                if print_construction:
                    print_node_info(
                        f"Branch {cols[selected_attribute]} = {value_int} (empty subset)",
                        level,
                    )
            ...
            if print_construction:
                print_node_info(
                    f"Branch {cols[selected_attribute]} = {value_int}", level
                )
    ...
    try:
        ...
        if tree is not None:
            ...
            if target_col in label_encoders:
                ...
                class_names = {
                    i: le.inverse_transform([i])[0] for i in range(len(le.classes_))
                }
            else:
                class_names = None

            evaluation_results = evaluate_decision_tree(
                tree, X_test, y_test, cols, class_names
            )
